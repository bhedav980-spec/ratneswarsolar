...
2. Open SQL Editor, create a new query, paste all of `supabase/SETUP.sql`, and run it once. It already contains every ordered migration through v11; do not run the individual migrations again on a new project. Supabase CLI users may run `supabase db push` instead.
...
7. Do not create public buckets. The SQL creates private business-document buckets. The private `agreement-files` bucket accepts the generated editable DOCX and remains protected by authenticated role policies.
