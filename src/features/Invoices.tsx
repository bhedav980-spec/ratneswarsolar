...
export function Invoices() {
  const { data, cancelCustomerInvoice, saving } = useCrm();
  ...
  return <>
    ...
    <section className="card table-card">{data.projects.length ? <table><thead><tr><th>Project</th><th>Customer</th><th>Capacity</th><th>Project Value</th><th>Invoice</th><th>Action</th></tr></thead><tbody>{data.projects.map((project) => {
      ...
      return <tr key={project.id}><td><strong>{project.projectNo}</strong><small>{project.stage.replaceAll('_', ' ')}</small></td><td><strong>{customer?.fullName ?? 'Archived customer'}</strong><small>{customer?.mobile}</small></td><td>{project.acceptedQuoteSnapshot.dcCapacityKw.toFixed(3)} kW<small>{project.acceptedQuoteSnapshot.panelBrand} {project.acceptedQuoteSnapshot.panelTechnology}</small></td><td><strong>{formatInr(project.acceptedQuoteSnapshot.grandTotal)}</strong><small>GST included</small></td><td>{invoice ? <Status tone="good">Issued {formatDate(invoice.invoiceDate)}</Status> : cancelledInvoice ? <Status tone="bad">Cancelled {formatDate(cancelledInvoice.invoiceDate)}</Status> : canIssueInvoice(project.stage) ? <Status tone="warn">Ready to issue</Status> : <Status tone="neutral">Waiting for installation</Status>}</td><td>{invoice ? <div className="row-actions">{customer&&<button className="btn btn--small btn--primary" onClick={() => setPreview({ invoice, project })}><Eye size={14} /> View / Print</button>}{data.profile.role==='admin'&&<button className="btn btn--small btn--danger" disabled={saving} onClick={async()=>{const reason=window.prompt(`Cancellation reason for ${invoice.invoiceNo} (required)`)??'';if(!reason.trim()||!window.confirm(`Cancel invoice ${invoice.invoiceNo}?`))return;await cancelCustomerInvoice(invoice.id,reason);setPreview(null);}}><XCircle size={14}/> Cancel Invoice</button>}</div> : cancelledInvoice ? <small>Delete the erroneous project from Projects.</small> : <button className="btn btn--small btn--primary" disabled={!canIssueInvoice(project.stage)} onClick={() => setIssuing(project)}><FilePlus2 size={14} /> Generate Invoice</button>}</td></tr>;
