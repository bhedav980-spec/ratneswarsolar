...
Final v10 adds internal official-price matching from the five verified PDFs, explicit WAAREE 540/580 and source-range wattage options, nearest-valid panel quantity calculation from requested kW, clean subsidy wording, and an audited Admin invoice-cancellation/project-cleanup flow. It preserves editable quotation values, loan pricing, BOM controls, material receipts and both GST-included and GST-extra invoice modes.
...
1. For a new Supabase project, read `SUPABASE_SETUP.md` and run the complete `supabase/SETUP.sql`. For the existing deployed project, follow `FINAL_V10_DEPLOYMENT.md` exactly.
...
For the exact existing-deployment sequence, start with `FINAL_V10_DEPLOYMENT.md`. See `ROLE_PERMISSION_MATRIX.md`, `DATABASE_SCHEMA.md`, `TEST_REPORT.md` and `BACKUP_AND_ROLLBACK.md` for production operations.
