...
function installInvoiceFonts(doc: jsPDF) {
  ...
  doc.addFileToVFS('InvoiceSans-Regular.ttf', regular);
  doc.addFont('InvoiceSans-Regular.ttf', PDF_FONT, 'normal');
  doc.addFont('InvoiceSans-Regular.ttf', PDF_FONT, 'italic');
  doc.addFileToVFS('InvoiceSans-Bold.ttf', bold);
  doc.addFont('InvoiceSans-Bold.ttf', PDF_FONT, 'bold');
}
...
function labeledCell(doc: jsPDF, x: number, y: number, w: number, label: string, value: string) {
  doc.setFont(PDF_FONT, 'normal'); doc.setFontSize(6.4); doc.text(clean(label), x + 1.2, y + 2.7);
  doc.setFont(PDF_FONT, 'bold'); doc.setFontSize(7.4);
...
function drawPartyAndMeta(doc: jsPDF, input: InvoicePdfInput, top: number, height: number) {
  ...
  let y = top + 4;
  doc.setFont(PDF_FONT, 'bold'); doc.setFontSize(9.2); doc.text(clean(company.legalName).toUpperCase(), x + 1.5, y);
  y += 3.2;
  ...
  doc.setFont(PDF_FONT, 'normal'); doc.setFontSize(6.8); doc.text('Buyer (Bill to)', x + 1.5, buyerTop + 3);
  const buyerNameLines = wrapped(doc, customer.fullName.toUpperCase(), half - 3).slice(0, 2);
  const buyerNameBottom = writeLines(doc, buyerNameLines, x + 1.5, buyerTop + 7, 8.4, 'bold', 3.2);
  let buyerY = buyerNameBottom + .3;
  ...
  const cells: Array<[string,string,string,string]> = [
    ...
    ['Place of Supply', invoice.placeOfSupply, 'Mode/Terms of Payment', quote.paymentTerms || settings.paymentTerms],
    ...
    ['Dispatched through', 'EPC Installation', 'Destination', '-'],
  ];
...
function drawItems(doc: jsPDF, input: InvoicePdfInput, top: number, height: number) {
  ...
  doc.setLineWidth(.14); doc.line(x, bodyBottom, x + width, bodyBottom);
  doc.setLineWidth(.11);
  positions.slice(1, -1).forEach((px) => doc.line(px, top, px, top + height));
  const headers = ['Sl No.', 'Particulars', 'HSN/SAC', 'Quantity', 'Amount'];
  doc.setFont(PDF_FONT, 'normal'); doc.setFontSize(7.3);
  ...
  doc.setFont(PDF_FONT, 'bold'); doc.setFontSize(8.5); doc.text('Total', positions[4]! - 1.5, bodyBottom + 6, { align: 'right' }); doc.setFontSize(10); doc.text(`INR ${money(invoice.grandTotal)}`, positions[5]! - 1.3, bodyBottom + 6, { align: 'right' });
}
...
function drawTaxSummary(doc: jsPDF, input: InvoicePdfInput, top: number) {
  ...
  if (invoice.igst > 0) {
    ...
    ['HSN/SAC','Taxable Value','IGST Rate','IGST Amount','Total Tax Amount'].forEach((h,i) => { doc.setFont(PDF_FONT,'normal'); doc.setFontSize(7); doc.text(h,pos[i]!+widths[i]!/2,top+4.5,{align:'center'}); });
...
export async function createInvoicePdf(input: InvoicePdfInput) {
  ...
  doc.rect(10.5,159,189,14); doc.setFont(PDF_FONT,'normal'); doc.setFontSize(6.8); doc.text('Amount Chargeable (in words)',11.7,162.5); doc.setFont(PDF_FONT,'italic'); doc.text('E. & O.E',198,162.5,{align:'right'}); doc.setFont(PDF_FONT,'bold'); doc.setFontSize(8.8); writeLines(doc,wrapped(doc,`INR ${amountInWords(invoice.grandTotal)}`,185),11.7,168,8.8,'bold',3.2);
  drawTaxSummary(doc,input,173);
  doc.rect(10.5,202,189,9); doc.setFont(PDF_FONT,'normal'); doc.setFontSize(7.4); doc.text('Tax Amount (in words):',11.7,207); doc.setFont(PDF_FONT,'bold'); doc.text(clean(`INR ${amountInWords(totalTax)}`),44,207);
  ...
  doc.setFont(PDF_FONT,'bold'); doc.setFontSize(7.5); if(signature){doc.saveGraphicsState();doc.setGState(doc.GState({opacity:.68}));doc.addImage(signature,'PNG',131.25,bottomTop+23,42,32.94,undefined,'NONE');doc.restoreGraphicsState();} doc.text(clean(`for ${company.legalName}`),197.5,bottomTop+34,{align:'right'}); doc.text('Authorised Signatory',197.5,bottomTop+53,{align:'right'});
