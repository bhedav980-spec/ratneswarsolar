...
-- ==================================================
-- 202607200015_agreement_feasibility_project_gate.sql
-- ==================================================
begin;
...
-- ==================================================
-- 202607200016_editable_feasibility_and_quote_signature.sql
-- ==================================================
begin;
...
-- ==================================================
-- 202607230017_quote_linked_and_manual_invoices.sql
-- ==================================================
begin;
...
-- ==================================================
-- 202607240018_compact_project_workflow.sql
-- ==================================================
begin;
...
-- ==================================================
-- 202608020019_idempotent_quotation_status.sql
-- ==================================================
begin;
...
-- ==================================================
-- 202608020020_recover_feasibility_project_creation.sql
-- ==================================================
begin;
...
-- ==================================================
-- 202608020021_retry_safe_invoice_issuance.sql
-- ==================================================
begin;

-- Invoice creation may be retried after a slow response, or after an earlier
-- invoice was cancelled while the installation material record and serials
-- remained attached to the same project. Keep those retries safe and audited.
create or replace function public.save_installation_and_issue_invoice(p_project_id uuid,p_details jsonb) returns uuid
language plpgsql security definer set search_path=public as $$
declare
 p projects%rowtype; q jsonb; expected int; serial_count int; duplicate_count int; tr tax_rules%rowtype;
 ...
 install_cgst numeric:=0; install_sgst numeric:=0; install_igst numeric:=0;
 taxable numeric; cgst_total numeric; sgst_total numeric; igst_total numeric; tax_lines jsonb:='[]'::jsonb;
 inv_id uuid:=gen_random_uuid(); inv_no text; base_inv_no text; reissue_no int;
 inv_date date:=coalesce(nullif(p_details->>'invoiceDate','')::date,current_date);
 place_ text:=coalesce(nullif(trim(p_details->>'placeOfSupply'),''),'Gujarat (24)'); serial text; panel_item uuid;
 existing_invoice customer_invoices%rowtype; existing_serial_project uuid;
begin
 select * into p from projects where id=p_project_id for update;
 if p.id is null or not can_access_project(p.id) or public.current_role()='dealer' then raise exception 'Not authorised'; end if;
 if not (p.current_stage = any(array['installation_done','inspection_pending','inspection_done','meter_pending','meter_done','commissioning_done','subsidy_pending','subsidy_passed','handover_completed','project_closed']::project_stage[])) then raise exception 'Invoice is available only after Project Installation Done'; end if;

 select * into existing_invoice from customer_invoices where project_id=p.id and status in('issued','paid') order by issued_at desc limit 1 for update;
 if existing_invoice.id is not null then return existing_invoice.id; end if;
 ...
 for serial in select jsonb_array_elements_text(p_details->'panelSerials') loop
  if nullif(trim(serial),'') is null then raise exception 'Panel serial numbers cannot be blank'; end if;
  select project_id into existing_serial_project from inventory_serials where serial_number=serial for update;
  if found and existing_serial_project is distinct from p.id then raise exception 'Installed serial number % belongs to another project',serial; end if;
  if found then
   update inventory_serials set inventory_item_id=panel_item,status='installed',project_id=p.id,updated_at=now() where serial_number=serial;
  else
   insert into inventory_serials(inventory_item_id,serial_number,status,project_id) values(panel_item,serial,'installed',p.id);
  end if;
 end loop;
 insert into installation_materials(project_id,details,created_by)
 values(p.id,p_details,auth.uid())
 on conflict(project_id) do update set details=excluded.details,updated_at=now();
 ...
 base_inv_no:=coalesce(nullif(trim(p_details->>'invoiceNo'),''),next_document_number('invoice','INV'));
 inv_no:=base_inv_no;
 select * into existing_invoice from customer_invoices where invoice_no=base_inv_no for update;
 if existing_invoice.id is not null then
  if existing_invoice.project_id<>p.id or existing_invoice.status<>'cancelled' then raise exception 'Invoice number already exists'; end if;
  select count(*)+1 into reissue_no from customer_invoices where project_id=p.id and status='cancelled';
  inv_no:=format('%s-R%s',base_inv_no,reissue_no);
  while exists(select 1 from customer_invoices where invoice_no=inv_no) loop reissue_no:=reissue_no+1;inv_no:=format('%s-R%s',base_inv_no,reissue_no);end loop;
 end if;
 insert into customer_invoices(id,invoice_no,customer_id,project_id,invoice_date,place_of_supply,status,tax_rule_id,quoted_amount,tax_treatment,taxable_value,cgst,sgst,igst,grand_total,snapshot,issued_by)
 values(inv_id,inv_no,p.customer_id,p.id,inv_date,place_,'issued',tr.id,quote_amount,tax_treatment,taxable,cgst_total,sgst_total,igst_total,gross,
  jsonb_build_object('id',inv_id,'invoiceNo',inv_no,'customerId',p.customer_id,'projectId',p.id,'invoiceDate',inv_date,'placeOfSupply',place_,'status','issued','taxMode',tax_treatment,'quotedAmount',quote_amount,'taxRuleName',tr.name,'taxLines',tax_lines,'taxableValue',taxable,'cgst',cgst_total,'sgst',sgst_total,'igst',igst_total,'roundOff',0,'grandTotal',gross),auth.uid());
...
revoke all on function public.save_installation_and_issue_invoice(uuid,jsonb) from public,anon;
grant execute on function public.save_installation_and_issue_invoice(uuid,jsonb) to authenticated;

commit;

-- ==================================================
-- 202608070022_invoice_signature_preference.sql
-- ==================================================
begin;
