    ...
    <div className="print-preview-stage"><article className="pdf-document-stack word-quote-stack">
      <section className="a4-document pdf-page word-quote-page">
        <Header/><main className="word-quote-body word-quote-page-one">
          ...
          <section><h2 className="word-section-title">DETAILS OF SUPPLY</h2><table className="word-supply-table"><thead><tr><th>Sr. No</th><th>Description of Goods</th><th>HSN Code</th><th>Qty</th><th>Unit Rate</th><th>Amount</th></tr></thead><tbody><tr><td>1</td><td><strong>Rooftop Solar System</strong> under {quote.scheme}</td><td>-</td><td>{quote.dcCapacityKw.toFixed(2)} kW</td><td>{formatInr(quote.grandTotal)}</td><td><strong>{formatInr(quote.grandTotal)}</strong></td></tr></tbody></table><div className="word-grand-total"><b>Grand Total (Including GST):</b><strong>{formatInr(quote.grandTotal)}</strong></div></section>
          <section className="word-subsidy"><h2 className="word-section-title">SUBSIDY DETAILS</h2><table><thead><tr><th>Sr. No</th><th>Description</th><th>Amount</th></tr></thead><tbody>{(quote.subsidy.referenceLines?.length?quote.subsidy.referenceLines:[{label:'Central Subsidy (Up to 2 kW)',amount:30000},{label:'Central Subsidy (Above 2 kW)',amount:18000},{label:'State Subsidy (Above 2 kW)',amount:30000},{label:'Agreement Charges',amount:350}]).map((line,index)=><tr key={line.label}><td>{index+1}</td><td>{line.label}</td><td>{formatInr(line.amount)}</td></tr>)}<tr><td></td><th>Total Subsidy</th><th>{formatInr(quote.subsidy.total||78000)}</th></tr></tbody></table></section>
