...
2. Open SQL Editor, create a new query, paste all of `supabase/SETUP.sql`, and run it once. Then run `supabase/migrations/202607170006_residential_price_list_source_ranges.sql` and `supabase/migrations/202607170007_operational_controls_and_settings.sql` in that order. Supabase CLI users may run `supabase db push` instead.
...
9. In Administration → Effective-Dated Tax and Subsidy Rules, publish the legally verified tax rule before issuing an invoice. No uncertain GST rate is hard-coded.
