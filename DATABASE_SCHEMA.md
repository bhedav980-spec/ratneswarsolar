...
Document numbers are allocated under row locks. `save_agreement_document` records the private editable DOCX. `save_feasibility_and_create_project` requires that agreement, stores the feasibility snapshot and creates exactly one project, material requirements/reservations and commission entry in one transaction. Direct authenticated execution of the legacy quotation-to-project function is revoked. `save_installation_and_issue_invoice` remains available at every post-installation stage, locks the project, validates serials and reverse-calculates tax without changing the accepted gross amount.
