...
-- Combined clean-project setup generated from ordered migrations.
...
create table public.customer_invoices (
 ...
 status text not null check(status in ('issued','paid','cancelled','credited')), tax_rule_id uuid references public.tax_rules(id), quoted_amount numeric(14,2) not null default 0, tax_treatment text not null default 'inclusive' check(tax_treatment in('inclusive','exclusive')), taxable_value numeric(14,2) not null, cgst numeric(14,2) not null default 0, sgst numeric(14,2) not null default 0, igst numeric(14,2) not null default 0, round_off numeric(14,2) not null default 0, grand_total numeric(14,2) not null,
...
-- Final split-GST invoice engine. This is included here so a clean Supabase
-- project needs only this SETUP.sql file; the ordered migration remains in
-- supabase/migrations for existing production databases.
...
values('Solar EPC 70/30 - Supply 5% / Installation 18%',date '2026-07-20',5,true,true,5,18,70,30,'854140','995442');

create or replace function public.save_installation_and_issue_invoice_split(p_project_id uuid,p_details jsonb) returns uuid
...
begin
 ...
 taxable:=supply_taxable+install_taxable; cgst_total:=supply_cgst+install_cgst; sgst_total:=supply_sgst+install_sgst; igst_total:=supply_igst+install_igst; gross:=supply_gross+install_gross;
...
grant execute on function public.save_installation_and_issue_invoice_split(uuid,jsonb) to authenticated;
commit;
...
-- v9 manual/automatic quotations, loan pricing, fixed dealer commission and material receipts.
...
-- Final workflow upgrade: direct quotation-to-project flow and persistent invoicing.
begin;
...
-- Final Area Partner hardening (2026-07-18).
begin;
create or replace function public.can_access_customer(p_customer uuid) returns boolean language sql stable security definer set search_path=public as $$
 select exists(select 1 from customers c where c.id=p_customer and c.archived_at is null and (is_admin() or (public.current_role()='district_partner' and c.assigned_partner_id=auth.uid()) or (public.current_role()='dealer' and c.dealer_id=current_dealer())))
$$;
create or replace function public.can_access_project(p_project uuid) returns boolean language sql stable security definer set search_path=public as $$
 select exists(select 1 from projects p where p.id=p_project and (is_admin() or (public.current_role()='district_partner' and p.assigned_partner_id=auth.uid())))
$$;
...
create policy dealers_read on public.dealers for select to authenticated using(is_admin() or id=current_dealer() or (public.current_role()='district_partner' and exists(select 1 from customers c where c.dealer_id=dealers.id and c.assigned_partner_id=auth.uid() and c.archived_at is null)));
drop policy if exists commissions_internal_read on public.dealer_commissions;
create policy commissions_internal_read on public.dealer_commissions for select to authenticated using(is_admin() or (public.current_role()='district_partner' and exists(select 1 from projects p where p.id=project_id and p.assigned_partner_id=auth.uid())));
drop policy if exists commission_payments_internal_read on public.dealer_commission_payments;
create policy commission_payments_internal_read on public.dealer_commission_payments for select to authenticated using(is_admin() or (public.current_role()='district_partner' and exists(select 1 from dealer_commissions c join projects p on p.id=c.project_id where c.id=commission_id and p.assigned_partner_id=auth.uid())));
insert into public.districts(code,name,active) values('PRIMARY','Primary Partner Area',true) on conflict(code) do update set active=true;
insert into public.profiles(id,full_name,role,district_id,dealer_id,active)
select u.id,coalesce(nullif(u.raw_user_meta_data->>'full_name',''),'Ratneswar Engineering Admin'),'admin',null,null,true from auth.users u where lower(u.email)=lower('ratneswarengineering@gmail.com')
...
insert into public.profiles(id,full_name,role,district_id,dealer_id,active)
select u.id,coalesce(nullif(u.raw_user_meta_data->>'full_name',''),'Area Partner'),'district_partner',d.id,null,true from auth.users u cross join public.districts d where lower(u.email)=lower('bhedav980@gmail.com') and d.code='PRIMARY'
...
do $$declare uid uuid;did uuid;dealer_uuid uuid;begin select id into uid from auth.users where lower(email)=lower('bhedavishal79@gmail.com');select id into did from public.districts where code='PRIMARY';if uid is not null then insert into public.profiles(id,full_name,role,district_id,dealer_id,active) values(uid,'Dealer User','dealer',did,null,false) on conflict(id) do update set role='dealer',district_id=did,dealer_id=null,active=false;select id into dealer_uuid from public.dealers where login_user_id=uid limit 1;if dealer_uuid is null then insert into public.dealers(dealer_no,name,mobile,email,address,district_id,login_user_id,default_commission_type,default_commission_value,active) values('DL-'||upper(substr(replace(uid::text,'-',''),1,8)),'Dealer User','PENDING-'||substr(uid::text,1,8),'bhedavishal79@gmail.com','Update from Dealer Master',did,uid,'fixed',0,true) returning id into dealer_uuid;end if;update public.profiles set dealer_id=dealer_uuid,active=true,suspended_at=null,suspended_reason=null where id=uid;end if;end $$;
commit;

-- Keep the split-GST implementation as the final callable invoice function.
...
language sql security definer set search_path=public as $$
 select public.save_installation_and_issue_invoice_split(p_project_id,p_details)
$$;
grant execute on function public.save_installation_and_issue_invoice(uuid,jsonb) to authenticated;
