...
export async function createInvoicePdf(input: InvoicePdfInput) {
  ...
  doc.setFont(PDF_FONT,'bold'); doc.setFontSize(7.5); doc.text(clean(`for ${company.legalName}`),197.5,bottomTop+34,{align:'right'}); if(signature)doc.addImage(signature,'PNG',114,bottomTop+28.3,35,27.45,undefined,'NONE'); doc.text('Authorised Signatory',197.5,bottomTop+53,{align:'right'});
